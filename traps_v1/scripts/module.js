    ...
    new Dialog({
        ...
        buttons: {
            place: {
                label: 'Place Trap',
                callback: async (html) => {
                    const trapType = html.find('[name="trapType"]').val();
                    ...
                    let trapPreview = null;
                    const preview = (event) => {
                        if (!trapPreview) {
                            trapPreview = new PIXI.Sprite(PIXI.Texture.from(trapType === 'ice-pit' ? 'icons/svg/ice-aura.svg' : 'icons/svg/trap.svg'));
