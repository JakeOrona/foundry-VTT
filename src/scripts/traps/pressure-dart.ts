 ...
 */
export async function handlePressureDartTrigger(
    trap: TrapConfig, 
    victimToken: Token, 
    victimActor: Actor,
    damage: number,
    ...
    let dartDirection: 'left' | 'right' | 'top' | 'bottom';
    if (minDist === distToLeft) dartDirection = 'right'; // Dart comes from left
    else if (minDist === distToRight) dartDirection = 'left'; // Dart comes from right
    else if (minDist === distToTop) dartDirection = 'bottom'; // Dart comes from top
    else dartDirection = 'top'; // Dart comes from bottom
